...
## 🌟 QRaw Exclusive Features

In addition to all the powerful features inherited from the upstream RapidRAW project, **QRaw** introduces several exclusive capabilities tailored for advanced workflows:

- **AI Chat Editing (ChatPanel):** Interact with an LLM-powered AI assistant directly within the editor to apply complex image adjustments through natural language commands.
- **i18n Multi-Language Support:** Fully localized interface with support for multiple languages (e.g., English, Simplified Chinese), making it accessible to a global user base.
- **Custom `.qcr` Config Logic:** Enhanced non-destructive sidecar file management utilizing a specialized `.qcr` configuration structure.
- **QRaw Brand Identity:** A completely white-labeled, independent brand identity built upon a robust open-source foundation.
...
- **Non-Destructive Workflow:** All edits stored securely.
...
git clone https://github.com/qiaopengg/QRaw.git
cd QRaw

# 2. Install frontend dependencies
npm install

# 3. Start the development server and open the client
