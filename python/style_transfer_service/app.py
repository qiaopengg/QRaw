...
def _run_single(
    ...
    if req.enable_refiner and _refiner is not None:
        # 🆕 进度：运行 Base Model
        _print_progress(20, f"运行 SDXL Base Model (步数: {steps})...", task_id)
        ...
        image = ref.images[0]
        _print_progress(90, "Refiner 完成", task_id)
    else:
        # 🆕 进度：运行单阶段推理
        _print_progress(20, f"运行 SDXL 推理 (步数: {steps})...", task_id)
        ...
        _print_progress(80, "推理完成", task_id)
    ...
    # 🔧 修复：归一化到 [0, 255]
    _print_progress(95, "后处理...", task_id)
...
def _run_tiled(
    ...
    while y0 < h:
        ...
        while x0 < w:
            ...
            _print_progress(progress_pct, f"处理第 {tile_count + 1}/{total_tiles} 块 (位置: {cx0},{cy0})", task_id)
            _debug_log(f"Processing tile {tile_count}: ({cx0}, {cy0}) -> ({x1}, {y1}), size: {actual_w}x{actual_h}")
            
            try:
                out = _run_single(reference_img, crop, req, task_id)
...
def style_transfer(req: StyleTransferRequest) -> StyleTransferResponse:
    ...
    try:
        result = _run_tiled(reference_img, content_img, req, task_id)
    except Exception as e:
        if str(e) == "warming_up":
