...
## 18.11 本轮记录（VLM 执行时机重大修正 - 让 VLM 看到预览图）

### AM. VLM 职责重新定义

当前结论：

- VLM 不再是"参数猜测者"，而是"整个流水线的最终质量把关者和审美优化专家"
- VLM 的核心职责分为两类：
  1. **纠错（Error Correction）- 必须执行**：检查并修正常见调色异常
  2. **审美微调（Aesthetic Refinement）- 谨慎执行**：轻微优化，单个参数调整幅度不超过±15

### AN. VLM 必须纠错的异常类型

当前结论：

VLM 作为专业摄影师质检员，必须检查并修正以下常见调色异常：
...
- 之前 VLM 只看参数 JSON，无法"看到"实际效果
- VLM 像"盲人医生"，只看化验单不看病人
- 无法直接识别死黑、死白、肤色异常等视觉问题

**解决方案：**

本轮已实施重大架构变更，让 VLM 能够"看到"调整后的效果：

1. 算法生成参数建议（步骤 1-7，进度 10%-70%）
2. **生成预览图**（步骤 8，进度 75%）← **新增关键步骤**
3. VLM 看三张图（步骤 9，进度 80%-95%）：
   - 参考图（风格目标）
   - 当前图（原始状态）
   - **预览图（应用算法建议后的效果）** ← **核心改进**
4. VLM 直接看到问题（死黑、死白、肤色异常等）
5. 返回纠错建议或放行
...
1. **`generate_preview_with_adjustments`**
   - 合并当前参数和算法建议
   - 生成完整的调整参数集
   - 调用 CPU 版本的参数应用

2. **`apply_cpu_adjustments_for_preview`**
   - CPU 版本的参数应用（不依赖 GPU）
   - 应用基础调整：曝光、对比度、饱和度等
   - 生成预览图供 VLM 分析

3. **`run_vlm_refinement` 函数签名修改**
   - 新增 `preview_img: &DynamicImage` 参数
   - VLM 现在接收三张图片而不是两张
   - 更新 user_text 说明三张图的含义

### AQ. VLM System Prompt 更新

当前结论：

VLM 的 system prompt 已完全重写，明确说明：

```
你是整个风格迁移流程的最终质量把关者和审美优化专家。
算法团队已经完成了初步分析，现在你需要：

## 核心职责

### 1. 纠错（Error Correction）- 必须执行
作为专业摄影师，你必须检查并修正常见调色异常...

### 2. 审美微调（Aesthetic Refinement）- 谨慎执行
在确保没有异常后，可以进行轻微的审美优化...

## 工作流程

1. 异常检测：仔细观察当前图，识别任何异常
2. 评估算法建议：检查算法建议是否会加重或引入新的异常
...
VLM 的 user_text 已更新，明确说明三张图的含义：

```
你现在看到三张图片：

1. 第一张图（参考图）：用户希望达到的风格目标
2. 第二张图（当前图）：原始状态，未应用任何调整
3. 第三张图（预览图）：应用算法建议后的效果
...
- 10%-70%：算法分析阶段
- **75%：正在生成预览图供视觉模型检查** ← 新增
- **80%：正在启动视觉大模型进行质量检查** ← 修改
- 80%-95%：VLM 分析阶段
- 95%：视觉模型微调完成
- 100%：分析完成
...
- ✅ 代码编译成功（`cargo check` 通过）
- ✅ 预览图生成函数已实现
- ✅ VLM 函数签名已更新
- ✅ 三张图片正确传递给 VLM
- ✅ System prompt 和 user_text 已更新
- ✅ 进度提示已同步更新
- ✅ 日志和前端显示已增强
...
1. 加载图片（10%）
   - 参考图、当前图、辅助参考图
   - 使用 canonical input 统一分析域

2. ViT-B Backbone 语义理解（15%-20%）
   - 提取参考图和当前图的语义 embedding
   - 计算语义相似度
   - 辅助参考图加权融合
...
10. VLM 质检（80%-95%）← **重大修改**
    - VLM 看三张图：参考图、当前图、预览图
    - 直接识别视觉异常（死黑、死白、肤色异常等）
    - 纠错优先，审美微调谨慎
    - 返回调整建议或放行
...
本轮架构变更的稳定性和安全性已通过以下方面验证：

**1. 编译时安全**

- 所有函数签名修改已通过 Rust 类型检查
- 无未处理的 Result 或 Option
- 无内存安全问题
...
**VLM 执行时机重大修正已完成并验证通过。**

这是 RapidRAW 分析式风格迁移架构的一次重大升级：

- VLM 从"参数猜测者"升级为"视觉质检员"
- 通过预览图让 VLM 能够"看到"实际效果
- 纠错能力和可靠性大幅提升
- 完全符合 V4 文档的分析式主路径原则
- 代码已编译通过，流程稳定可靠
