...
pub fn compute_clip_quality(image: &DynamicImage, clip: &ClipModels) -> Option<f64> {
    ...
    if encodings.is_empty() { return None; }
    ...
    let ids_array = ndarray::Array::from_shape_vec((n, max_len), ids_data).ok()?.into_dyn();
    let mask_array = ndarray::Array::from_shape_vec((n, max_len), mask_data).ok()?.into_dyn();
...
fn preprocess_clip_image(image: &DynamicImage) -> Array4<f32> {
    ...
    let resized = image.resize_to_fill(input_size, input_size, image::imageops::FilterType::Triangle);
