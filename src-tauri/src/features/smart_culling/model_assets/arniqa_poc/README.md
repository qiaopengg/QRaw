...
2026-08-20 使用 `manual-reference` / `source` 既有 200 张校准集做了一次严格隔离的 Core ML POC，未写星且未接入生产链。该集合已参与历史调参，**不是独立盲测**，以下数字只用于淘汰明显不可行路线：

- 全图 SPAQ / KonIQ / KADID 与人工星级的 Spearman 分别为 `0.0841 / 0.0614 / -0.0635`；仅看眼态已开放的人像仍只有 `0.0765 / 0.0142 / 0.0516`。全图路线无法支撑星级或清晰门禁，已拒绝接入。
- 人物 ROI（人脸框外扩 15%）中，SPAQ 对 6 张已知明显运动模糊与人工 3–5 星对照的区分 AUC 为 `0.9948`，但开眼子集与最终星级的 Spearman 仅 `0.1823`。样本只有 6 个明确模糊正例，且 ROI 缩放并非官方训练契约，因此只能保留为“人物清晰辅助证据”候选，不得据此生产化。
- 当前优先使用已有原始像素清晰门禁解决这 6 张漏判；ARNIQA ROI 仍需新的按拍摄任务切分的独立盲测，并且许可、QRaw 运行时和 Windows 门禁全部通过后才可重新考虑。
